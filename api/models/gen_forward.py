...
class GenForward:
    ...
    def generate_wav(self):
        ...
        tts_weights = None
        tts_load_path = (
            tts_weights if tts_weights else self.paths.forward_latest_weights
        )
        ...
        for i, x in enumerate(inputs, self.alpha):
            ...
            _, m, dur, pitch = tts_model.generate(
                x, alpha=self.alpha, pitch_function=pitch_function
            )
            ...
            if self.input_text:
                save_path = (
                    ...
                    / f"{self.input_text[:10]}_{self.alpha}_{v_type}_{tts_k}k_amp{self.amp}.wav"
                )
            else:
                save_path = (
                    self.paths.forward_output
                    / f"{i}_{v_type}_{tts_k}k_alpha{self.alpha}_amp{self.amp}.wav"
                )
